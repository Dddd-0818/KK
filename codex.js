...
============================================================ */

    {
      ...
      keywords: '群像,剧情,群戏,多角色,状态栏,正则,世界书,杀青,开场白,时间感知,配色,CSS,占位符,状态面板,导演',
      description: '群像剧情是一个让 AI <b>同时扮演多个角色</b>、像导演一样推动一整场群戏的模式。它和单聊最大的不同：一条 AI 消息里会同时出现旁白、好几个角色的对白与动作，像在读一部小说 / 看一场戏。<br><br><b>整体流程</b><br>① 在面具墙选一个你的人设（面具）<br>② 进入工作区，去「选角」勾几个角色 → 建一个群像<br>③ 进入群像聊天页，写开场或直接点发送让 AI 起头<br>④ 用底部「＋」号里的各种工具（时间感知 / 世界书 / 杀青 / 配色 / 自定义CSS / 状态面板）把这场戏调成你想要的样子<br><br>下面每一项都有详细说明，重点看「状态面板」那条，自定义 UI 全靠它。',
      ...
      features: [
        ...
        {
          id: 'gs-status',
          title: '⑧ 状态面板 · 自定义 UI（重点）',
          keywords: '状态栏,状态面板,正则,占位符,自定义,UI,模板,placeholder,Observer,内心,吐槽,首字母,nav,导演,html',
          description: '这是群像最强的功能：<b>点角色头像，弹出一张状态卡</b>，显示这条消息发生时——大家在哪、几点、每个角色的身份/状态/上帝吐槽/内心独白。而且这张卡的<b>样式完全由你自定义</b>，AI 只负责填内容。<br><br>━━━━━━━━━━━━━━<br><b>一、它是怎么工作的（先理解原理）</b><br>━━━━━━━━━━━━━━<br>这套用的是「<b>占位符</b>」机制：<br>① 你写一份 HTML 模板，在要填数据的地方插<b>占位符</b>（如 <code>{{char_status}}</code>）<br>② AI 每条消息按约定输出一段隐藏的状态数据（用户看不到原始标记）<br>③ 系统把 AI 的数据替换进你模板的占位符里 → 渲染出「你的样式 + AI 的内容」<br><br>这样<b>样式归你、内容归 AI</b>，互不打架。你想做成杂志风、游戏面板、病历卡，随便。<br><br>━━━━━━━━━━━━━━<br><b>二、怎么开启 + 改模板</b><br>━━━━━━━━━━━━━━<br>1) 底部＋号 → 状态面板<br>2) 打开「<b>启用状态面板</b>」开关（关着就不占 token）<br>3) 中间有一份<b>可编辑的 HTML 模板</b>（默认就是那张杂志风卡），上面有<b>实时预览</b>，改模板当场看效果<br>4) 点「默认模板」可以恢复成自带的那张卡<br>5) 保存<br><br>开启后，AI 会自动为每个在场角色生成状态。聊天页里有状态数据的角色，头像右下角会有个小圆点，<b>点头像</b>就弹出状态卡，顶部圆点切换不同角色。<br><br>━━━━━━━━━━━━━━<br><b>三、占位符清单（写模板用这些）</b><br>━━━━━━━━━━━━━━<br>在你的 HTML 模板里，把这些 <code>{{...}}</code> 放在想显示对应内容的位置：<br><br><b>全局（整场戏共用一份）</b><br>· <code>{{loc_cn}}</code> — 当前地点中文名<br>· <code>{{loc_en}}</code> — 当前地点英文/拼音<br>· <code>{{time}}</code> — 当前时间（如 23:45 PM）<br><br><b>角色（每个角色一份，切换圆点时变）</b><br>· <code>{{char_initial}}</code> — 角色首字（系统自动算：中文取首字、英文取首字母大写）<br>· <code>{{char_name_cn}}</code> — 角色中文名（系统自动填）<br>· <code>{{char_name_en}}</code> — 角色英文名/拼音（AI 填）<br>· <code>{{char_role}}</code> — 身份/头衔（AI 填）<br>· <code>{{char_status}}</code> — 当前一句话状态（AI 填）<br>· <code>{{god_note}}</code> — 上帝视角吐槽（AI 填，≤20字，第三人称毒舌）<br>· <code>{{thought}}</code> — 内心独白（AI 填，≤30字，第一人称）<br><br><b>特殊占位符</b><br>· <code>{{nav}}</code> — <b>角色切换圆点</b>。系统会把所有在场角色的圆点填到这里。把它放在你想让圆点出现的位置（比如卡片顶部）。多角色才显示，单角色自动隐藏。如果模板里不写 <code>{{nav}}</code>，圆点会自动兜底显示在卡片上方。<br><br>━━━━━━━━━━━━━━<br><b>四、自己写模板的注意事项</b><br>━━━━━━━━━━━━━━<br>1) 模板就是一段<b>普通 HTML</b>，可以带 <code>&lt;style&gt;</code> 标签写样式（默认模板就是这么做的）<br>2) 想加新字段？目前占位符是固定这几个（上面清单），AI 只认这些。你只能改它们的<b>排版和样式</b>，不能凭空加「好感度」这种新数据字段（那需要改提示词和解析逻辑）<br>3) 占位符不区分大小写敏感，但请<b>原样照抄</b>（如 <code>{{char_status}}</code>，别写成 <code>{{ char_status }}</code> 带空格）<br>4) 改坏了不要紧，点「默认模板」就还原<br>5) 内心和吐槽的字数有上限（30/20字），系统会自动截断并去掉多余括号，所以<b>布局不会被超长文字撑坏</b><br><br>━━━━━━━━━━━━━━<br><b>五、举个最简单的自定义例子</b><br>━━━━━━━━━━━━━━<br>如果你只想要一张极简卡，把模板整个换成这样就行：<br><br><code>&lt;div style="padding:20px;background:#fff;border-radius:12px;font-family:serif;"&gt;<br>&nbsp;&nbsp;{{nav}}<br>&nbsp;&nbsp;&lt;h2&gt;{{char_name_cn}}（{{char_role}}）&lt;/h2&gt;<br>&nbsp;&nbsp;&lt;p&gt;📍{{loc_cn}} · {{time}}&lt;/p&gt;<br>&nbsp;&nbsp;&lt;p&gt;状态：{{char_status}}&lt;/p&gt;<br>&nbsp;&nbsp;&lt;p style="color:#999"&gt;旁白吐槽：{{god_note}}&lt;/p&gt;<br>&nbsp;&nbsp;&lt;p style="color:#5D6B78"&gt;内心：{{thought}}&lt;/p&gt;<br>&lt;/div&gt;</code><br><br>系统会自动把 AI 的数据填进去。看懂这个，你就能照着默认模板改出任何样式了。',
          action: () => (window.GroupStoryModule && GroupStoryModule.open && GroupStoryModule.open())
        },
