...
const UpdateModule = (() => {
  ...
  const CHANGELOG = [
"✦ 动态新功能【角色能在动态里发语音了】：角色评论你的动态时，现在能直接发语音条了，复用 MiniMax 音色引擎，每个角色按各自配置的音色说话、互不串音；长留言会逐句合成，自然度跟单聊一致，不再是整段念稿的机器音。语音条做成唱片样式，点黑胶即播、自动展开转写文字兜底。外语语音同样只朗读原文、不读翻译括号。（按角色独立开关，在 ta 的单聊设置里开启；没配音色或没开的角色仍发纯文字。）",
"✦ 存储优化【一键揪出占地大图】：备份页新增「存储瘦身」区。「扫描并清理无用图片」会列出已删角色/聊天残留的孤儿图，逐张勾选、默认先打包备份再清理，不误伤在用的图；「大图排行」按体积从大到小排出最占地的前 30 张，清楚标注每张是「在用」还是「可清理」，一屏看穿空间都堆在哪。同时修复了删除角色时未连带清理其聊天记录、导致配图残留占空间的问题。",
