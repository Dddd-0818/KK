...
const UpdateModule = (() => {
 ...
 const CHANGELOG = [
  "✦ 角色档案新增【角色导入 · KiKi/txt/docx】：在新建档案页底部，可上传 KiKi 导出的备份包 (.zip/.json)，勾选要搬入的角色即可，头像一起带过来；勾一个直接填进表单方便调整，勾多个则批量入库。也支持从文档 (.txt/.docx) 导入人设，正文整段进人设、图片自动忽略。仅迁移角色档案，聊天记录无法跨项目迁移。",
  "✦ 单人剧情新增【网易云配乐】：+ 菜单里填好自己的网易云 API 和 cookie 并开启，AI 就会随剧情情绪自动点歌，配乐做成票根风播放条，点一下即可播放。注意：开配乐时记得开🪄，否则抓不到歌。操作和群像的配乐一样。",
  "✦ 数据备份新增【精简导出】：可只导出角色 + 用户面具 + 聊天记录（含相关头像图片），导入仍用同一个恢复按钮，会自动识别并只覆盖这几类数据，不动其他子系统。"
];
